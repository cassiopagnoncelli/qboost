testthat::test_that("qrb fits and predicts", {
  ...
  fit <- qrb(x, y, tau = 0.4, nrounds = 25, nfolds = 2, early_stopping_rounds = 5)
  preds <- predict(fit, x[1:5, ])

  testthat::expect_s3_class(fit, "qrb")
...
testthat::test_that("summary works", {
  ...
  fit <- qrb(x, y, tau = 0.5, nrounds = 20, nfolds = 2, early_stopping_rounds = 5)
  s <- summary(fit)

  testthat::expect_s3_class(s, "qrb_summary")
...
testthat::test_that("formula interface fits and predicts", {
  ...
  fit <- qrb(y ~ x1 + x2, data = df, tau = 0.35, nrounds = 20, nfolds = 2, early_stopping_rounds = 5)
...
testthat::test_that("plots are returned as ggplot list", {
  ...
  fit <- qrb(x, y, tau = 0.25, nrounds = 15, nfolds = 2, early_stopping_rounds = 3)
...
testthat::test_that("importance returns a data frame", {
  ...
  fit <- qrb(x, y, tau = 0.7, nrounds = 15, nfolds = 2, early_stopping_rounds = 3)
  imp <- importance.qrb(fit)
...
testthat::test_that("minimal data still fits", {
  ...
  y <- rnorm(10)
  fit <- qrb(x, y, tau = 0.5, nrounds = 5, nfolds = 2, early_stopping_rounds = 2)
