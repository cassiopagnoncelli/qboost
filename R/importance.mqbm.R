...
#'
#' Aggregates feature importance across all symbol-specific qbm models.
...
#'   \item{gain}{Average information gain across all symbol models}
#'   \item{sd_gain}{Standard deviation of information gain across symbols}
#'   \item{n_models}{Number of symbol models where this feature appears}
#'   
#' @details
#' For each feature, this function:
#' \enumerate{
#'   \item Extracts feature importance from each symbol-specific qbm model
#'   \item Computes the mean information gain across all symbols
#'   \item Computes the standard deviation of information gain
#'   \item Counts how many symbol models use this feature
#' }
#' 
#' Features are ranked by mean information gain in descending order.
#' Features that don't appear in all symbol models will have their gain
...
#' 
#' fit <- mqbm(y ~ x1 + x2 + x3, data = df, tau = 0.5, nrounds = 50)
#' 
...
importance.mqbm <- function(object, ...) {
  ...
  # Extract importance from each symbol-specific model
  imp_list <- lapply(object$symbols, function(sym) {
    imp <- importance(object$models[[sym]], ...)
    if (nrow(imp) == 0) {
      return(NULL)
    }
    imp$symbol <- sym
  ...
  # For each feature, compute mean and sd of gain across symbols
  result <- lapply(all_features, function(feat) {
    # Get gains for this feature from all symbols
    feat_gains <- imp_combined[imp_combined$feature == feat, ]
    
    # Create a vector with one gain per symbol (0 if feature not present)
    gains_by_symbol <- numeric(length(object$symbols))
    names(gains_by_symbol) <- object$symbols
    
    for (i in seq_along(object$symbols)) {
      sym <- object$symbols[i]
      # Extract gain as a numeric value
      sym_rows <- feat_gains[feat_gains$symbol == sym, ]
      if (nrow(sym_rows) > 0) {
        gains_by_symbol[sym] <- as.numeric(sym_rows$gain[1])
      } else {
        gains_by_symbol[sym] <- 0
      }
    }
    
    tibble::tibble(
      feature = feat,
      gain = mean(gains_by_symbol, na.rm = TRUE),
      sd_gain = stats::sd(gains_by_symbol, na.rm = TRUE),
      n_models = sum(gains_by_symbol > 0)
    )
  })
