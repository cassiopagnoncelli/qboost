...
#' @param top_n Maximum number of features to show in the importance plot.
#' @param ... Unused, included for compatibility.
#'
#' @return A list of `ggplot` objects (invisible if printed).
#' @export
#' @method plot qboost
plot.qboost <- function(x, plot = TRUE, top_n = 25, ...) {
  ...
  plots <- list(
    ...
    importance = plot_feature_importance(imp_df, top_n = top_n)
  )
