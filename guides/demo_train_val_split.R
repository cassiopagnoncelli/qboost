...
cat("Fitted values on training set available:", 
    all(!is.na(fitted(fit_split, type = "surface")[train_idx])), "\n")
cat("Fitted values on validation set are NA:", 
...
fit_folds <- qbm(y ~ x1 + x2, data = df, tau = 0.5, nrounds = 50,
                 folds = custom_folds)
...
for (sym in fit_mqbm$symbols) {
  ...
  cat(sprintf("  %s: %d training obs, %d validation obs in global split\n",
...
fit_mqbm_folds <- mqbm(y ~ x1 + x2, data = df_multi, tau = 0.5, nrounds = 50,
